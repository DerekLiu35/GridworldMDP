    ...
    while True:
        ...
        for s in mdp.get_states():
            if not mdp.is_terminal(s):
                utils = [sum([mdp.get_successor_probs(s, a)[succ] * old_utilities[succ] for succ in mdp.get_successor_probs(s, a).keys()]) for a in mdp.get_actions(s)]
                expected_util = gamma * max(utils)
                utilities[s] = mdp.get_reward(s) + expected_util
                delta = max(delta, abs(utilities[s] - old_utilities[s]))
        if (epsilon > delta):
            break
